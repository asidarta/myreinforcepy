...
def generate_directions():
    for m in range(0, nfile+1):

        fid = open('%s/exper_design/%s/%s%d.txt'%(mypwd,subj,subj,m), 'aw')
        fid.write("[\n")
        ...
        for i in range (1,tot+1):
            ...
            # (6) Construct a dict for each trial, then append it to a list. 
            # IMPORTANT: Each time you have to create a new list if not what you
            # add is just the reference to the dict.
            myString = "{ \"trial\":%d,\"anchors\":%s,\"probe\":%d,\"delay\":%d }"%(i,temp,probe,delay)
        
            # (7) Save the file in JSON format
            #print myString
            if not i == tot: fid.write(myString +",\n")
